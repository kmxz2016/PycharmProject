...
#include "sixrobot_grasping/grasping_demo.h"
    ...
    vMng_(length, breadth)
{
  ...
  image_sub_ = it_.subscribe("/sixrobot_anno/camera/image_raw", 1, &GraspingDemo::imageCb, this);
}
...
int main(int argc, char **argv)
{
  ...
  if (!n.getParam("sixrobot_grasping/table_length", length))
    length = 0.3;
